...
class initProgFrame extends JFrame {
	...
	private JButton commitButton = new JButton("提交");
	...
	public initProgFrame() {
		...
		sqlSelectLabel = new JLabel("请选择使用的数据库类型：");
		...
		referButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int ifcontinue = JOptionPane.showConfirmDialog(null, "请确认单据过账",
						"单据确认", JOptionPane.YES_NO_OPTION,
	...
	public void transmitMain(int selectedrow) {
		try {
			...
			JOptionPane.showMessageDialog(null,
					"数据未能写入，请检查是否正确设置数据库，如依旧有问题请与作者联系");
...
class InitDateBase {
	public InitDateBase() {
		int IFCONFIRM = JOptionPane.showConfirmDialog(null,
				"本操作会删除现有的所有数据且不可恢复，请慎重选择", "警告", JOptionPane.OK_CANCEL_OPTION,
